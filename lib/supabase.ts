...
export async function getFoodsByDiningHall(diningHallId: number): Promise<FoodItem[]> {
  try {
...
export async function getCommonFoodItems(): Promise<FoodItem[]> {
  try {
    const { data, error } = await supabase.from("food_items").select("*").eq("is_dining_hall_food", false).order("name")
...
export async function createMeal(
  ...
  try {
    // Insert the meal
    const { data: mealData, error: mealError } = await supabase.from("meals").insert(meal).select("id").single()
...
export async function getUserMeals(userId: string, limit = 10): Promise<MealWithFoodItems[]> {
  try {
    // First get the meals
    ...
    const mealsWithFoodItems: MealWithFoodItems[] = await Promise.all(
      meals.map(async (meal) => {
        ...
        if (mfError) {
          console.error(`Error fetching food items for meal ${meal.id}:`, mfError)
...
export async function getDailyNutritionSummary(userId: string, date: string): Promise<DailyNutritionSummary | null> {
  try {
    const { data, error } = await supabase
